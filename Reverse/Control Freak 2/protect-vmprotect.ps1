param(
    ...
    [string]$LinuxProject = "vmprotect\chall-3-linux.vmp",
    [string]$WindowsProject = "vmprotect\chall-3-windows.vmp",
    [switch]$InPlace
)
...
$targets = @(
    @{
        Input = Join-Path $root "dist\chall-3"
        Output = Join-Path $root "dist\chall-3.vmp"
        Project = Join-Path $root $LinuxProject
    },
    @{
        Input = Join-Path $root "dist\chall-3.exe"
        Output = Join-Path $root "dist\chall-3.exe.vmp"
